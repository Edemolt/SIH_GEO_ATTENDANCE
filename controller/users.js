...
// writing something diff... lets see
export const register = async (req, res) => {
  try {
    const { uuid, email, name, password, currentLocation, workMode = "WFO" } = req.body;
    ...
    const user = await prisma.user.create({
      data: {
        ...
        currentLocation,
        workMode,  // Save workMode as WFO/WFH
      },
    });
    res.json(user);
  } catch (err) {
...
export const updateTimestamp = async (req, res) => {
  ...
  try {
    ...
    } else if (user.workMode === "WFH") {
      // For WFH, handle taking breaks and resuming work
      if (entered === "true") {
        // Resuming work from a break
        updateData.inCount = {
          increment: 1,
        };
      } else {
    ...
    res.json({ status: 'OK', updatedTimes: updatedUser[field] });
  } catch (error) {
    console.error('Error updating timestamp:', error);
